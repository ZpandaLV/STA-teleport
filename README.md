-- =============================================================================
-- SCRIPT EXECUTOR - APOCALYPSE UTILITY V5.3 (CLEAN FIXED)
-- =============================================================================
...
MainFrame.Draggable = true -- Mengizinkan menu digeser di layar HP
MainFrame.ClipsDescendants = true -- Memotong isi konten saat menu mengecil (Minimize)
...
-- Tombol Lock Floating (Gembok agar menu tidak sengaja tergeser)
...
--------------------------------------------------------------------------------
-- 2. LOGIKA OPERASIONAL (FUNGSI FITUR LOKAL)
--------------------------------------------------------------------------------

-- Perbaikan Animasi Minimize Mulus (On/Off Tampilan Konten)
...
MinimizeButton.MouseButton1Click:Connect(function()
	...
	if isMinimized then
		ContentFrame.Visible = false -- Sembunyikan tombol saat menu mengecil
	end
	...
	tween.Completed:Connect(function()
		if not isMinimized then
			ContentFrame.Visible = true -- Tampilkan kembali saat menu membesar sempurna
		end
	end)
end)

-- Fitur Kunci Posisi Menu (Lock Drag)
...
-- =============================================================================
-- SERVER SCRIPT: LOGIKA DAMAGE & DATA ZOMBIE (ALWAYS ON)
-- (Taruh di ServerScriptService - Hanya untuk Game Sendiri di Roblox Studio)
-- =============================================================================

local Players = game:GetService("Players")

-- Daftar pengali kerusakan berdasarkan tipe zombie (Disesuaikan dari Screenshot_2026-06-19-19-05-27-18.jpg)
...
-- Fungsi utama pengeksekusi damage resmi
local function applyApocalypseDamage(attacker, victimHumanoid, weaponType)
	...
	-- Menentukan damage dasar senjata
	...
	-- Mengambil atribut jenis zombie, default kembali ke Walkers jika tidak ada atribut
	local targetType = victimHumanoid.Parent:GetAttribute("ZombieType") or "Walkers"
	
	-- Mengambil nilai multiplier langsung tanpa pengecekan tombol ON/OFF
	...
	-- Terapkan pemotongan darah target secara resmi di server
	victimHumanoid:TakeDamage(finalDamage)
	
	print(string.format("[Combat Server] %s memberi %d damage ke %s (%s)", attacker.Name, finalDamage, victimHumanoid.Parent.Name, targetType))
end

-- Pemicu serangan (Hubungkan ke fungsi deteksi hit/kontak dari senjata Anda)
...
-- Metode Amblas Tanah ke Base (Glitch Lokal)
...
-- Teleport Instant ke Player Lain
